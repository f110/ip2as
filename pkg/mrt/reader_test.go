...
func TestReader_Scan_PeerIndexTable(t *testing.T) {
...
func TestReader_Scan_RIBIPv4Unicast(t *testing.T) {
	// Timestamp = 0x5d1d4180
	// Type = 0x000d (13 - TableDumpv2)
	// SubType = 0x0002 (2 - RIBIPv4Unicast)
	// Length = 0x00000095 (149)
	input := []byte{
		,
	}
	r := NewReader(bytes.NewReader(input))
	if !r.Scan() {
		t.Fatal("expect true but got false")
	...
	msg := r.Message()
	unicast, ok := msg.(*TableDumpv2RIB)
	if !ok {
		t.Fatal("expect TableDumpv2RIB but not")
	}
	if unicast.SeqNumber != 0 {
		t.Error("incorrect Sequence Number")
	}
	if unicast.EntryCount != 4 {
		t.Log(unicast.EntryCount)
		t.Error("incorrect Entry Count")
	}
	if len(unicast.RIBEntries) != int(unicast.EntryCount) {
		t.Fatalf("mismatch between Entry Count and RIBEntries length (%d vs %d)", unicast.EntryCount, unicast.RIBEntries)
	}
}
